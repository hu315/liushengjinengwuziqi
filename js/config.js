...
// 技能按钮ID映射
window.BTN = {blast:"btnBlast",copy:"btnCopy",sediment:"btnSed",thunder:"btnThunder",domain:"btnDomain",cannon:"btnCannon",sitKill:"btnSitKill",gambler:"btnRps"};
...
// 难度描述
window.DIFF_DESC = [
    "AI仅堵五连，高失误率，不用技能","AI会堵活四，低概率用简单技能","AI攻守平衡，正常使用技能",
    "标准AI，低失误率，技能有逻辑","零失误AI，优先防守，技能精准","预判2步，进攻性增强，技能更频繁",
    "高级AI，活三活四全封堵，玩家技能-1","大师级AI，攻守无破绽，玩家首回合禁技能",
    "职业级难度，全局最优解，玩家沉淀上限-1","地狱级难度，无解进攻+完美防守"
];
...
window.WHEEL_EFFECTS = [
    ...
    {name:"恢复2次技能",fn:p=>{ window.D[p].c = Math.min(window.getMaxCount(p,window.D[p].s), window.D[p].c+2); return "恢复2次技能"; }},
    {name:"本回合跳过",fn:p=>{ return "本回合跳过"; }},
    {name:"落子+1技能+1",fn:p=>{ window.extra += 1; window.D[p].c = Math.min(window.getMaxCount(p,window.D[p].s), window.D[p].c+1); return "落子+1技能+1"; }},
    {name:"削弱效果",fn:p=>{
        if(window.D[p].se > 0){ window.D[p].se = 0; return "沉淀层数清零"; }
        window.D[p].c = Math.max(0, window.D[p].c - 1); return "扣除1次技能次数";
    }}
];